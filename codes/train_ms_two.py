...
def validate(val_loader, opt, model, current_step, epoch, logger):
    ...
    for val_data in val_loader:
        ...
        save_img_path = os.path.join(img_dir, 'HyperRIM_{:s}_{:s}_{:s}_x4_{:d}.png'.format( \
...
def main():
    ...
    for epoch in range(num_month):
        for i, train_data in enumerate(train_loader):
            # get the code
            if use_dci:
                cur_month_code = get_code_for_data_two_lpips(model, train_data, opt)
            else:
                code_val_0 = torch.randn(train_data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
                                         train_data['LR'].shape[2], train_data['LR'].shape[3])
                code_val_1 = torch.randn(train_data['LR'].shape[0], int(opt['network_G']['in_code_nc']),
